...
- کتابخانه انیمیشن مجاز: Framer Motion (ورود/میکرواینترکشن) + **GSAP ScrollTrigger** برای موشن‌های scrubbed اسکرولی — نصب با تأیید صریح سپهر ۱۴۰۵/۰۴/۲۴
...
- **قانون طلایی موشن scrubbed: فقط GSAP ScrollTrigger (scrub) — نه useScroll/useTransform موشن.** نسخه motion حتی بعد از memo هم در مرورگر واقعی فریز می‌کرد (لایه وسط کراس‌فید یخ می‌زد/قدم آخر نمی‌آمد)؛ تایم‌لاین GSAP هر فریم وضعیت را قطعی از progress ست می‌کند و این کلاس باگ را ناممکن می‌کند. motion فقط برای ورود/میکرو بماند
- تست عددی scrubbed در پنل: ticker پنل کند است و سمپل «یک قدم عقب» می‌دهد — قبل از خواندن opacity ها ~۲.۵ ثانیه صبر کن؛ اعداد ScrollTrigger (start/end) را با محاسبه دستی مقایسه کن برای تشخیص واقعی
